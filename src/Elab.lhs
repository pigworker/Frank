...
> getOps :: [Sig] -> ElabM [OpDecl]
> getOps [] = return []
> getOps (Pure : _) = return []
> getOps (s@(u :$ vs) : ss) = do
>   info <- getInfoE
>   (| ~[  (ot, (n, xsub vs ois, (s : xsub vs oss), xsub vs oo))
...
> opCType :: (Template -> HD) -> OpDecl -> ElabM (TM {SYN}, CT)
> opCType w (t, (m, is, ss, o)) = do
>   ws <- fresh m
>   sigs <- getSigsE
>   return (H (w t), sAct (xsub ws (foldr (:->) (F ss o) is)) sigs)
...
>     Nothing -> case [tt | tt@(f', _) <- ops, f == f'] of
>       (_:_:_) -> moanE $ AmbiguousOp f sigs
>       [tt] -> do -- trace ("OPTYPE " ++ show tt) $
>         (h, ct) <- opCType OP tt
>         use h [] ct ts
>       [] -> case lookup f (funDecls info) of
>         Just ft -> do
>           (h, ct) <- opCType FN (f, ft)
>           use h [] ct ts
...
> skolemize :: Int -> ElabM [VT]
> skolemize m = do
>     (g, n) <- getCxE
>     setCxE (grow g n m)
>     return (map (X . (negate . (+ 1))) (take m [n ..]))
>   where
>     grow g n 0 = (g, n)
>     grow g n m = grow (g :< Skolem (negate (n + 1))) (n + 1) (m - 1)
...
> patCh (M os v)  (PRet p)    = (|(id *** PR) (patCh v p)|)
> patCh (M os v)  (POp ps p)  = patTree ps [] >>= \ z -> case z of
...
>       [(m, us, os, u)] -> do
>         ws <- skolemize m
>         (gz, ps)  <- ttpsCh o (xsub ws us) ts
>         (hz, p)   <- patCh (U (xsub ws u :-> F (sAct (xsub ws os) sigs) v)) p
