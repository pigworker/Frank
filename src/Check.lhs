...
> type OpDecl = (Template, (Int, [VT], [Sig], VT))

> data TypeInfo = TypeInfo
>   {  dataDecls  :: [(UpId, (Int, [(Template, [VT])]))]
>   ,  sigDecls   :: [(UpId, (Int, [OpDecl]))]
>   ,  funDecls   :: [OpDecl]
...
>       sg (n, (xs, os)) = (| ~n, (| ~(length xs), traverse (opTypeInfo xs) os|)|) where
>       opTypeInfo xs stuf@(vs, (ss, v)) = do
>         let ys = ftvs xs src stuf
>         let xys = xs ++ ys
>         (t, vs') <- conTemplate src xys vs
>         ss' <- tScope src xys ss
>         v' <- tScope src xys v
>         return (t, (length ys, vs', ss', v'))
>   ds <- (| ~ primitiveTypes ++ traverse dt (sourceData src) |)
>   ss <- (| ~ primitiveSigs ++ traverse sg (sourceSigs src) |)
>   fs <- (| ~primitiveFunDecls ++ traverse (opTypeInfo []) (sourceDecl src) |)
...
> ftvs :: TMang t => [UpId] -> Source -> t -> [UpId]
> ftvs xs src = ala' UnionList (ala' Const tMang) (var chk mempty mempty)
>   where chk u = if elem u xs then [] else
>                   maybe (maybe [u] (const []) (lookup u primitiveTypes))
...
>   tScope src xs (M os v) = (|M (tScope src xs os) (tScope src xs v)|)
...
> primitiveSigs :: [(UpId, (Int, [(Template, (Int, [VT], [Sig], VT))]))]
> primitiveSigs =
>   [  (Up "Console", (0,
>      [  ([Mark "inch"], (0, [], [], char))
>      ,  ([Mark "ouch"], (0, [char], [], Un))
